...
const getEmailTemplate = (content, previewText = '', platformName = 'DonateFit') => `
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
const sendEmail = async (to, subject, htmlContent, previewText = '', throwError = false) => {
  try {
    const platformName = await getSetting('platform_name') || 'DonateFit';
    const cleanSubject = subject.replace(/Donate Fate|DonatePlate|DonateFate/gi, platformName);
    let cleanHtmlContent = htmlContent.replace(/Donate Fate|DonatePlate|DonateFate/gi, platformName);
    let cleanPreviewText = previewText.replace(/Donate Fate|DonatePlate|DonateFate/gi, platformName);
...
module.exports = {
  ...
  sendEmailVerificationEmail: (email, name, verifyUrl) =>
    sendEmail(
      email,
      'Verify your email address — DonateFit',
      `
        <h3 style="color: #0f172a; font-size: 20px; margin-top: 0; margin-bottom: 16px;">Almost there, ${name}! 🎉</h3>
        <p style="margin: 0 0 16px;">Thanks for signing up for DonateFit. Before you can log in, we need to confirm your email address.</p>
      ...
      `,
      'Confirm your email to activate your DonateFit account.'
    ),

  sendWelcomeEmail: (email, name) =>
    sendEmail(
      email,
      'Welcome to DonateFit!',
      `
        <h3 style="color: #0f172a; font-size: 20px; margin-top: 0; margin-bottom: 16px;">Welcome, ${name}!</h3>
        <p style="margin: 0 0 16px;">Your email has been verified and your DonateFit account is now fully active.</p>
  ...
  sendAdminVerifiedEmail: (email, name) =>
    sendEmail(
      ...
      `
        ...
        <p style="margin: 0 0 16px;">An administrator has manually reviewed and verified your DonateFit account.</p>
  ...
  sendDonationAlertEmail: (creatorEmail, creatorName, donorName, amount, campaignTitle) =>
    sendEmail(
      creatorEmail,
      `You just received a $${amount} donation on DonateFit!`,
      `
  ...
  sendTestEmail: (email) =>
    sendEmail(
      email,
      'Test Email - DonateFit SMTP Configuration',
      `
        <h3 style="color: #0f172a; font-size: 20px; margin-top: 0; margin-bottom: 16px;">Success! 🎉</h3>
        <p style="margin: 0 0 16px;">If you are reading this, it means your SMTP configuration for DonateFit is working perfectly.</p>
  ...
  sendPasswordResetEmail: (email, resetUrl) =>
    sendEmail(
      ...
      `
        ...
        <p style="margin: 0 0 16px;">We received a request to reset your password for your DonateFit account.</p>
      ...
      `,
      'Instructions to reset your DonateFit password.'
    ),
  ...
  sendUserBannedEmail: (email, name, banType, banExpiresAt, reason) => {
    ...
    return sendEmail(
      email,
      'Important notice regarding your DonateFit account',
      `
        <h3 style="color: #ef4444; font-size: 20px; margin-top: 0; margin-bottom: 16px;">Account Suspended</h3>
        <p style="margin: 0 0 16px;">Hello ${name},</p>
        <p style="margin: 0 0 16px;">We are writing to inform you that your DonateFit account has been suspended.</p>
        ${reasonSection}
      ...
      `,
      'Your DonateFit account has been suspended.'
    );
  },
  ...
  sendUserUnbannedEmail: (email, name) =>
    sendEmail(
      email,
      'Your DonateFit account has been restored',
      `
        <h3 style="color: #10b981; font-size: 20px; margin-top: 0; margin-bottom: 16px;">Account Restored! 🎉</h3>
        <p style="margin: 0 0 16px;">Hello ${name},</p>
        <p style="margin: 0 0 16px;">We are pleased to inform you that the suspension on your DonateFit account has been lifted, and your account has been fully restored.</p>
      ...
      `,
      'Your DonateFit account suspension has been lifted.'
    ),
};
